...
// =========================================================
// 🌟 MULTI-BLOCK PARSER (FIXED COLUMN MAPPING)
// =========================================================
    ...
    try {
        ...
        if (cloudData.assignments && cloudData.assignments.length > 1) {
            cloudData.assignments.slice(1).forEach(row => {
                ...
                // 🌟 FIX: The 'Class Teacher' column is actually at Column F (Index 5), not H!
                let classTeacherClass = String(row[5] || '').trim(); 

                // Block 1 is always at Columns C, D, E (Indices 2, 3, 4)
                let blocks = [
                    { act: row[2], cls: row[3], per: row[4] }
                ];
                
                // 🌟 FIX: Subsequent blocks start at Column G (Index 6)
                // We use i + 2 < row.length to prevent reading incomplete blocks (like a "Total" column at the end)
            ...
            allTeachers.forEach((teacher) => {
                ...
                daysOfWeek.forEach((day, dIdx) => {
                    ...
                    teachingPeriods.forEach(period => {
                        ...
                        if (slot) {
                            // 🌟 REFINEMENT: Increased character limit for subjects to prevent awkward "ECONOMI C" cuts.
                            let shortSub = slot.subjectName.length > 11 ? slot.subjectName.substring(0, 11) + '..' : slot.subjectName;
                            // 🌟 REFINEMENT: Remove spaces from complex class strings (like 12-A&B&B 2) so they don't break onto multiple lines weirdly.
