...
/**
 * Parses Class-Sections supporting standard combinations using '&' and ','
 * Input Example: "12-A&B, 11-C" -> Output: ["12-A", "12-B", "11-C"]
 */
function getIndividualClasses(classNameStr) {
    if (!classNameStr) return [];
    let result = [];
    
    // Split by comma first to separate distinct groups (e.g., "12-A&B, 11-C")
...
function generateAutoTimetable() {
    ...
    // 🌟 Aggressive Priority Assignment Order
        ...
        for (let i = 0; i < req.periodsPerWeek; i++) {
            ...
            for (let d = 0; d < daysOfWeek.length; d++) {
                ...
                for (let period of teachingPeriods) {
                    // Strict Rule: Class Teacher owns Period 1
    ...
    try {
        ...
        if (cloudData.assignments && cloudData.assignments.length > 1) {
            cloudData.assignments.slice(1).forEach(row => {
                ...
                let classTeacherClass = String(row[7] || '').trim(); // Based on new sheet structure, Class Teacher is in H (index 7)

                // 🌟 REFINEMENT: Explicit mapping for the first block based on your spreadsheet layout
                // Activity (C), Class (D), Periods (E) -> indices 2, 3, 4
                let blocks = [
                    { act: row[2], cls: row[3], per: row[4] }
                ];
                
                // Add subsequent blocks starting from Column F (index 5)
                ...
                blocks.forEach(block => {
                    ...
                    if (!activity || !classSecStr) return; 

                    // 🌟 REFINEMENT: Handle multiple distinct classes in a single block (e.g. "12-A, 11-C")
                    ...
                    distinctClasses.forEach(distinctClassGroup => {
                        ...
                        if (finalPeriods > 0) {
                            ...
                            if (classTeacherClass) {
                                // Extract just the grade and section from the class teacher string for accurate matching
                            ...
                            SCHOOL_CONFIG.assignments.push({
                                ...
                                className: distinctClassGroup, // Pass the group (e.g., "12-A&B")
