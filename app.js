 ...
 * Features: Absolute Determinism, Combined Class Splitter (with & support), 
 * Row-per-Teacher Multiblock Parsing, Automatic Period Calculation, Class Load Tracking UI.
 */
...
const APP_CONFIG = {
    ...
    shortName: "GHSS VMPT",                                           
    scriptUrl: "https://script.google.com/macros/s/AKfycbxB5QPuyap0M3wkb9OC8L8w6HupStKGgff5RRA_-cgu_9xz88_9kxPZxJio3oEitQ_1Jg/exec" 
};
...
/**
 * Parses Class-Sections supporting standard combinations using '&' (e.g., "8-A&B")
 */
...
function generateAutoTimetable() {
    ...
    // 🌟 Aggressive Priority Assignment Order: Highest weekly load gets scheduled first
        ...
        for (let i = 0; i < req.periodsPerWeek; i++) {
            ...
            for (let d = 0; d < daysOfWeek.length; d++) {
                ...
                for (let period of teachingPeriods) {
                    // Strict Rule: Class Teacher owns Period 1 exclusively for that class
    ...
    // Update real-time summary indicators in UI
    updateClassLoadUI();
}
...
// =========================================================
// 🌟 HORIZONTAL HORIZON MULTI-BLOCK PARSER
// =========================================================
    ...
    try {
        ...
        if (cloudData.assignments && cloudData.assignments.length > 1) {
            cloudData.assignments.slice(1).forEach(row => {
                let teacherName = String(row[1] || '').trim(); // Column B: Teacher Name
                if (!teacherName) return; 

                let classTeacherClass = String(row[2] || '').trim(); // Column C: Class Teacher details

                // Dynamic Block Engine: Step size 3 parsing columns horizontally starting at index 3 (Activity 1)
                for (let i = 3; i < row.length - 1; i += 3) {
                    let activity = String(row[i] || '').trim();
                    let classSec = String(row[i+1] || '').trim();
                    let periodsVal = String(row[i+2] || '').trim();

                    if (!activity || !classSec) continue; 

                    let gradeVal = getGradeValue(classSec);
                    let finalPeriods = 0;

                    // Support default 'AUTO' configuration with programmatic custom overrides
                    ...
                    if (finalPeriods > 0) {
                        // Resolve if this assigned workload belongs to their specific Class Teacher class
                        let isCT = (classTeacherClass && classSec.includes(classTeacherClass)) ? true : false;
