 ...
 * Fixes: Two-Pass Smart Distribution (No dropped periods), Strict Parsing, PDF Abbreviation
 */

const APP_CONFIG = {
    fullName: "GHSS VELAMURITHANPETTAI", 
    shortName: "GHSS VMPT",                                           
    scriptUrl: "https://script.google.com/macros/s/AKfycby8MMqLz0uoPoBuzTnLs4AdY4Z080NLd2iD5A92OrltQwIyQScuawG2lei5iSO_KCv-cQ/exec" 
};
...
// --- CORE TIMETABLE GENERATOR (TWO-PASS DISTRIBUTION) ---
function generateAutoTimetable() {
    ...
    // Provide a standardized placement function for both passes
    function attemptPlacement(req, reqIndex, allowLimitBypass) {
        ...
        for (let i = 0; i < req.periodsPerWeek; i++) {
            if (req.assignedCount >= req.periodsPerWeek) break; // Already satisfied
            ...
            for (let offset = 0; offset < daysOfWeek.length; offset++) {
                ...
                // If strict mode, enforce limit. If bypass mode, allow stacking to fulfill requirement.
    ...
    // Initialize tracking property
    SCHOOL_CONFIG.assignments.forEach(req => req.assignedCount = 0);

    // PASS 1: Strict distribution to ensure even spread
    SCHOOL_CONFIG.assignments.forEach((req, reqIndex) => {
        attemptPlacement(req, reqIndex, false);
    });

    // PASS 2: Mop-up unassigned periods by relaxing daily limits
    SCHOOL_CONFIG.assignments.forEach((req, reqIndex) => {
        if (req.assignedCount < req.periodsPerWeek) {
            attemptPlacement(req, reqIndex, true);
        }
    });
